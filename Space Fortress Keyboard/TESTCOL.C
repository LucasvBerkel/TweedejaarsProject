...
Test_Collisions(int level_indicator)
{
  int breakflag;
  int i;
  ...
  if((Mine_Flag==ALIVE)&&(level_indicator > 1))
  ...
  if((Shell_Flag==ALIVE) && (!breakflag) && (level_indicator > 0) )
  ...
  if((!Missile_Vs_Mine_Only)&&(level_indicator > 0))
    if(Missile_Flag[i]==ALIVE)
	 if(Check_Collision(Missile_X_Pos[i],Missile_Y_Pos[i],
