...
Save_Game_Results(int Save_To_File)
{
  header.Number_Of_Planned_Games=No_Of_Games;
  header.One_Game_Duration=One_Game_Duration;

  game.Score=Score=Points+Velocity+Control+Speed;
  game.Points=Points;
  game.Velocity=Velocity;
  game.Control=Control;
  game.Speed=Speed;
  game.No_Of_Bonus_Intervals=No_Of_Bonus_Intervals;
  game.No_Of_Points_Bonus_Taken=No_Of_Points_Bonus_Taken;
  game.No_Of_Missiles_Bonus_Taken=No_Of_Missiles_Bonus_Taken;
  game.No_Of_Ship_Damaged_By_Fortress=Ship_Damaged_By_Fortress;
  game.No_Of_Times_Ship_Damaged_By_Mines=Ship_Damaged_By_Mines;
  game.No_Of_Times_Fortress_Distroyed_By_Trainee=Fortress_Destroyed;
  game.Normal_Game_Termination=Normal_Game_Termination;

  if(Save_To_File) write_file(filename,Game_Counter,1);
    /* after first game Game_Counter=1 */

  final_display();
  return(0);
}

Run_Session(int Training_Session)
{
...
Announce_Session_End()
{
  ...
  gprintf(&x,&y,"GAME IS OVER !");
  x=0.25*MaxX; y=0.8*MaxY;
  gprintf(&x,&y,"<press escape to exit>");
  setcolor(svcolor); /* restore previous color */
  return(0);
}
...
Save_Aiming_Game(int Training_Session)
{
   header.Number_Of_Planned_Games=No_Of_Games;
   header.One_Game_Duration=One_Game_Duration;
   ...
   if(Training_Session)
     {
  write_file(filename,Game_Counter,0);
    /* 0 - for aim_sess_results */
    /* after first game Game_Counter=1 */
     }
     return(0);
}
...
Run_Aiming(int Training_Session)   /* 1- for training 0- for demo */
{
  ...
  do
   ...
   if ( (elapsed_time=Time_Counter-loop_start_time) < 50)
    ...
    } while((!End_Flag)&&(Loop_Counter < One_Game_Duration*1200));
  /* ESC or three minutes */
  ...
  Save_Aiming_Game(Training_Session);
