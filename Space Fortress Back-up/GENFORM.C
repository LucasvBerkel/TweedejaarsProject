...
extern void show_error(char *note);
...

{
  ...
  FILE	 		*fileptr;
   ...
   {
      show_error("Not enough memory to allocate buffer");
  ...
  if (fileptr==0) show_error("error writing file");
  ...
  if ((form_rec = malloc(size_in_bytes)) == NULL)
   {
      show_error("Not enough memory to allocate buffer"); /* terminate program if out of memory */
   }
  else
  {
   if (tcu_write_formrec(&form, form_rec)==TCU_OK )
   for (i=0; i < size_in_bytes; i++) fputc(form_rec[i], fileptr);
   else  show_error("Writing form-record to buffer");
   free(form_rec);
  }
...

{
  ...
  if (tcu_read_formrec(&form, form_rec)==TCU_ERROR)
     show_error("error in reading form-record to buffer");
