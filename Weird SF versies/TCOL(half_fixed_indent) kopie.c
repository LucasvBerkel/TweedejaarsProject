...
void Test_Collisions(cairo_t *cr)
{
  ...
  if(Game_Type==AIMING_TEST)
  {
    ...
    {                  /* check against mine only */
       if(Mine_Flag==ALIVE && Missile_Flag[i]==ALIVE)
      {
         if(Check_Collision(Missile_X_Pos[i],Missile_Y_Pos[i],        Mine_X_Pos,Mine_Y_Pos,COLLISION_DIST))
         {
          Missile_Flag[i]=KILL;
...
//      TCOL.c that use old headers
  ...
  if(Mine_Flag==ALIVE)
	...
	{
		if(Check_Collision(Ship_X_Pos,Ship_Y_Pos, Shell_X_Pos,Shell_Y_Pos,COLLISION_DIST) )
	  {
	    Ship_Killings_Counter++;
  ...
  if(!breakflag)
    ...
    /******** misile vs. fortress *********/
			...
			if(!Missile_Vs_Mine_Only)
			{
				...
				{
					// MaxX/2,MaxY/2 is the location of the fortress 
					if(Check_Collision(Missile_X_Pos[i],Missile_Y_Pos[i],MaxX/2,MaxY/2,COLLISION_DIST) )
					{
						Missile_Flag[i]=KILL;
						Handle_Missile_Flag=ON;
