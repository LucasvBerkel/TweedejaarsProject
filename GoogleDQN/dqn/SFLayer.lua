...
require 'torch'
local game = torch.class('SF_Game')
require 'paths'
...
--    self.useRAM   = options.useRAM
    ...
    self.name = gamename
    local path_to_game = paths.concat(roms_path, gamename) .. '.so'
    ...
    self.game_over = function() return self.env:isTerminal() end
...
function game:shape()
	print("Warning: the shape() function might not be implemented correctly")
...
function game:saveState()
	error("Saving states is not implemented")
--    self.env:saveState()
end
...
function game:getState()
	print("Warning: snapshotting is not implemented.")
